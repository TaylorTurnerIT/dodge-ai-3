"""Measure learning quality for collector and CUDA optimization factors on T4."""
...
TREATMENTS = {
    "lanes8-baseline": {
        "run_id": "t4-best-lane8-baseline-200k-s43-v1",
        "collector_lanes": 8,
        "collector_execution": "parallel",
        "learner_backend": "baseline",
    },
    "lane1-optimized": {
        "run_id": "t4-best-lane1-opt-200k-s43-v1",
        "collector_lanes": 1,
        "collector_execution": "serial",
        "learner_backend": "cuda-optimized",
    },
    "lane1-baseline": {
        "run_id": "t4-best-lane1-baseline-200k-s43-v1",
        "collector_lanes": 1,
        "collector_execution": "serial",
        "learner_backend": "baseline",
    },
...
def run(treatment: str, history: Path, *, smoke: bool) -> Path:
    ...
    revision = os.environ.get("DDQN_SOURCE_COMMIT", "unknown")[:7]
    run_id = (
        f"{spec['run_id']}-smoke-{revision}" if smoke else str(spec["run_id"])
    )
    root = train_run(
        history_root=history,
        run_id=run_id,
        steps=64 if smoke else 200_000,
        seed=43,
        stack_size=4,
        ...
        collector_lanes=int(spec["collector_lanes"]),
        collector_execution=str(spec["collector_execution"]),
        learner_backend=str(spec["learner_backend"]),
        device="cuda",
        dueling=True,
        learning_rate=1e-4,
    ...
    record = {
        "experiment": "collector-x-learner-quality-ablation-v1",
        "treatment": treatment,
        ...
        "collector": {
            "lanes": spec["collector_lanes"],
            "execution": spec["collector_execution"],
        },
        "eval_batch_size": 16,
        "learner_backend": spec["learner_backend"],
        "telemetry": "existing log interval 1000; no added per-step diagnostics",
    }
