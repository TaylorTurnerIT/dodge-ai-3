...
AA1|active|implementation|Mean,max,learnedattention,4×4grid pooling; shared localdecoder; sourcechecked references; fresh-T4 launcher restoring retained inputs with hash gates, hash-gated recovery, separate CUDA smoke|Z4|Luna max plus parentreview; tests/Ruff; freeze beforefitting
AA2|pending|input verification|Restore exact standard/spatial banks onto fresh T4 from retained bytes into immutable input root; verify world/data/frame/palette/array hashes pre-fit; no extraction except hash-gated recovery of Phase-0-missing files (stage, verify exact full digests, then promote; mismatch stops before fitting)|AA1|Same16384train/2048validation, train-derived3colorpalette; reject drift
...
AA3|pending|fitting|Four freshheads512/2048; frozen256×192patchinput; sharedcoreinit904/sample903/batch32/AdamW0.001 decay0.01/rawCE|AA2|One7200sT4worker; sameZcoreinitialhash; attentionadds192trainablepoolweights, recorded separately
AA4|pending|evaluation|Alltrain/validation,wrongframe,meanimage; squaregallery; compare Z matched512/2048; checkpoints/sampler/hash audits|AA3|Retrieve verifiedarchive, inspect images, report limitations, releaseT4
...
AC3|pending|AA as compression screen|Keep bounded arms+controls; grid4 is 16×192=3072 retained values not a 192 test; all-arms-fail cannot prove every 192 code inadequate (already AA.limits)|AC1|Compare vs Z matched milestones; drop 192-insufficiency decision rule
