...
AA1|active|implementation|Mean,max,learnedattention,4×4grid pooling; shared localdecoder; sourcechecked references; reusableT4 launcher withproxyrefresh|Z4|Luna max plus parentreview; tests/Ruff; freeze beforefitting
AA2|pending|input verification|Reuse exact standard/spatial banks fromZ; verify world/data/frame/palette/array provenance; no extraction|AA1|Same16384train/2048validation, train-derived3colorpalette; reject drift
...
AC2.result|lewm-actual-decode-20260916-v1 third T4 launch after B71 device fix; 2 heads×3 inputs matrix over 16 scenes + 1536 varied-offset windows (0.25/0.5/0.75), retrieved/verified. Matched actual-trained (primary) vs §AB predicted-trained (reference): scenes actual-input classErr 0.3021 vs 0.3128 (reference on predicted reproduces §AB 0.3142); broad 0.3120 vs 0.3247. Best cell primary_on_predicted 0.2986. Every cell within ~2.6pp and at wrong-latent control 0.3089; pixel persistence 0.0072 dominates all. Readout mismatch is not the bottleneck: small consistent matched-actual advantage on actual inputs; target-space decodability remains weak, consistent with AC1. Does not prove/disprove H2.
