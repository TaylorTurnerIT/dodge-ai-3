...
AD.stages|Stage1: 1024→20000 total updates in ~4-8k chunks per T4 session with checkpoint resume; AC1/AC2 probes at 5k/10k/20k on frozen intermediate checkpoints. Stage-1a (1024→5120) runs the Y2 envelope (batch32/float32); stage-1b (5120→20000) switches to batch128/bf16 per R7 paper envelope — loss-curve discontinuity recorded, probes compare within-envelope. Advance to paper-scale (~160k ≈ 10 epochs over 524288 train transitions) only if prediction:persistence ratio improves stage-over-stage; stop rule: ratio flat across two consecutive probe points.
