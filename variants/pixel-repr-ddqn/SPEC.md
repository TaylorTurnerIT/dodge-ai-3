...
AD.stages|Stage1: 1024→20000 total updates in ~4-8k chunks per T4 session with checkpoint resume; AC1/AC2 probes at 5k/10k/20k on frozen intermediate checkpoints. Advance to paper-scale (~160k ≈ 10 epochs over 524288 train transitions) only if prediction:persistence ratio improves stage-over-stage; stop rule: ratio flat across two consecutive probe points.
...
AD2|active|collection|Expanded determined-scenario corpus, hashed+frozen|AD.authorization|New families cover validation transition gaps; seeds disjoint; no learner updates.
