...
AD5|pending|diagnostic fitting/evaluation|Action-conditional control eval: frozen survival-cost probe (train split only) + 1-step greedy MPC (9 actions, read-only world) rolled out headless on train-seed vs novel-seed scenario sets; survival steps vs scripted/random baselines|AD4|P6 diagnostic, not P7: no world-model updates, no policy learning, no optimization. Seeds disjoint per V3; env variants, not copies.
