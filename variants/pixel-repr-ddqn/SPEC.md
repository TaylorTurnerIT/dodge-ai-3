...
V57|Decode evaluation metric math stays CPU/NumPy: only forward-pass inputs move to the compute device; decoder outputs detach to CPU before any metric arithmetic; no target/image tensor moves to the compute device in evaluation paths. CPU-only tests cannot see cross-device crashes, so a device-move spy asserts no image-rank tensor is moved in _decode_cells.
...
AC2|active|actual-next-latent decoder|Train ONLY the new actual-next projected-z head on §AB's exact midpoint training windows/targets/arch/init904/sample903/loss/2048budget; evaluate saved AB head + new head on original 16 scenes AND broader varied-offset windows with cross-head matrix and FP-aware metrics|AC1|Training-window match keeps one-variable comparison; varied training offsets would need a new protocol identity
