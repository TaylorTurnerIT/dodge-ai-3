...
AB1|active|implementation|Mid-episode window selection, teacherforced one-step ẑ features, predicted readout via shared localdecoder, persistence/wrong-latent controls, five-panel gallery with bottomright diffmap, T4 worker+launcher with proxyrefresh|Z4|Tests/Ruff; freeze commit before fitting
AB2|pending|fitting|Fresh head on ẑ train windows, one deterministic mid-episode window per train episode (all4096); 512/2048; coreinit904/sample903/batch32/AdamW0.001 decay0.01; balanced-bright MSE (0.5 cream, 0.5 other, per-frame-then-batch)|AB1|World hash unchanged before/after; window/episode provenance recorded
AB3|pending|evaluation+view|16 validation scenes: current observed/decoded, next observed/decoded, diffmap; predicted vs latent-persistence vs pixel-persistence baselines|AB2|Inspect images; changed-region/per-color metrics; limitations explicit
